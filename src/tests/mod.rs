use crate::{parse_ips, Record, Result};
...
fn parse_single_normal() -> Result<()> {
    ...
    assert_eq!(record, vec![Record::Normal { offset: 0x426900, data: Vec::from([0, 0]) }]);
    Ok(())
}
...
fn parse_single_rle() -> Result<()> {
    ...
    let record = parse_ips(ips)?.into_iter().collect::<Vec<_>>();
    assert_eq!(record, vec![Record::RLE { offset: 0x426900, size: 0x420, data:0xFF }]);
    Ok(())
}
